...
function App() {
  ...
  const handleSendMessage = useCallback(async (rawText: string) => {
    ...
    try {
      ...
      // Convert conversation history into Gemini format, skipping the very first default message to save context if desired, or keeping it.
      // We will just send the latest user prompt with a strong system instruction for now.
